...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
    ...
    int area = height * width; // Caltculates the area of the grid

    float belief_per_cell = 1.0 / area; //Calculates the belief for each cell for the new grid

    vector< vector <float> > newGrid;
    for (int i = 0; i < height; i++) { //Robot "walks" through height
        vector<float> row;
        for (int j = 0; j < width; j++) { //Robot "walks" through width
            row.push_back(belief_per_cell); //inserts new element at the end of vector
        }
        newGrid.push_back(row); //inserts new element at the end of vector
    ...
    float blurring)
{
    ...
    for (int i = 0; i < beliefs.size(); i++) {
        ...
        for (int j = 0; j < beliefs[i].size(); j++) {
            ...
            int new_i; // To store two integer values
            int new_j; // To store two integer values

            new_i = (((i + dy) % height) + height) % height; //the modulo expression
            new_j = (((j + dx) % width) + width) % width; // the modulo expression
